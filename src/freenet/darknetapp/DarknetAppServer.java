...
public class DarknetAppServer implements Runnable {
    ...
    private final int DEFAULT_PORT = 7858; 
    ...
    private int maxDarknetAppConnections =100;	
    ...
    public DarknetAppServer(SubConfig darknetAppConfig, Node node, Executor executor) {
        ...
        System.out.println("DarknetApoo - 1");
        /*try {
            change(darknetAppConfig);
    ...
    private class darknetAppEnabledCallback extends BooleanCallback  {
		@Override
		public Boolean get() {
			synchronized(DarknetAppServer.this) {
				return myThread != null;
			}
		}
		@Override
    ...
    private static String l10n(String key) {
		return NodeL10n.getBase().getString("DarknetAppServer."+key);
	}
    @Override
    public void run() {
        try {
    ...
    public class SocketHandler implements PrioRunnable {
		...
		public void run() {
			...
			try {
				DarknetAppConnectionHandler.handle(sock);
